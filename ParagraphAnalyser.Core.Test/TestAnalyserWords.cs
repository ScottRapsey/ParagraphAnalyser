...
namespace ParagraphAnalyserTests
{
    ...
    public class TestAnalyserWords
    {
        ...
        {
            var result = Analyser.GetWordsGroupedByFirstChars("A");
        ...
        {
            var result = Analyser.GetWordsGroupedByFirstChars("A. A.", ignoreCase: false);
            Assert.AreEqual(1, result.Count());
            Assert.AreEqual('A', result.First().Key);
        ...
        {
            var result = Analyser.GetWordsGroupedByFirstChars("A. a.", ignoreCase: true);
            Assert.AreEqual(1, result.Count());
        ...
        {
            var result = Analyser.GetWordsGroupedByFirstChars("Albuquerque. Albuquerque.", ignoreCase: false);
            Assert.AreEqual(1, result.Count());
            Assert.AreEqual('A', result.First().Key);
            Assert.AreEqual(2, result.First().Count());
        }
        ...
        {
            var result = Analyser.GetWordsGroupedByFirstChars("Albuquerque. albuquerque.", ignoreCase: true);
            Assert.AreEqual(1, result.Count());
            Assert.AreEqual('A', result.First().Key); //could legitimately return 'a'
            Assert.AreEqual(2, result.First().Count());
        }
        ...
        public void LongParagraphTest()
        {
            ...
            var result = Analyser.GetWordsGroupedByFirstChars(text, ignoreCase: true);
